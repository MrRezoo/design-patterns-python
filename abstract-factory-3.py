...
# ----------------------------------------------------------
def order_suv(corp,
              model):  # corp is the company, model is the model of the car
    if issubclass(model, corp):
        # check if the model is related to corp, so you cannot call M2 from Benz
...
try:
    ...
    # calling Corporation incorrectly will raise NameError, calling Model
    # incorrectly will raise AttributeError
